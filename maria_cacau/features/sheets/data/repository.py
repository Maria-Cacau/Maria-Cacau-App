...
class SheetsRepository:
    def connect(self, link: str, name: str) -> Sheet:
        ...
        return Sheet(name=name, sheet_id=sheet_id)

    def select(self, sheet_id: str) -> Sheet:
        SelectSheetAPI(sheet_id).call()
        sheets = self._load_raw()
        raw    = next((s for s in sheets if s["sheet_id"] == sheet_id), None)
        return Sheet(name=raw["nome"] if raw else sheet_id, sheet_id=sheet_id)
    ...
    def find_by_link(self, link: str) -> Sheet | None:
        ...
        return Sheet(name=raw["nome"], sheet_id=raw["sheet_id"]) if raw else None

    def load_all(self) -> list[Sheet]:
        return [Sheet(name=s["nome"], sheet_id=s["sheet_id"]) for s in self._load_raw()]
    ...
    def auto_select_last(self) -> Sheet | None:
        ...
        return Sheet(name=last["nome"], sheet_id=last["sheet_id"])
